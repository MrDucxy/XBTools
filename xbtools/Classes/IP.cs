...
namespace xbtools.Classes
{
    class IP
    {
        ...
        public string IP_GetCpuKey(string ip, int saveDir = 0)
        {
            ...
            else if (saveDir == 1) folder = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "output");
        ...
        private void scanLiveHosts(string ipFrom, string ipTo, ProgressBar pb)
        {
            ...
            while (ipLong < to)
            {
                ...
                MainWindow.mainWindow.Dispatcher.BeginInvoke(new Action(() => pb.Value = i));
                //Console.Write("\r{0}%   ", i);
            ...
            MainWindow.mainWindow.Dispatcher.BeginInvoke(new Action(() => pb.Value = pb.Maximum));
        }
        ...
        public void IPScanner(ProgressBar pb)
        {
            ...
            MainWindow.mainWindow.Dispatcher.BeginInvoke(new Action(() => pb.Value = pb.Minimum));
            foreach (string o in ip)
            {
                ...
                MainWindow.mainWindow.Dispatcher.BeginInvoke(new Action(() => pb.Value = pb.Value + (100 / (ip.Count + 1))));
