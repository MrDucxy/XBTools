...
namespace xbtools.Classes
{
    public class xFlasher
    {
        ...
        public bool osCheck()
        {
            ...
            {
                MessageBox.Show("This version of Windows is not supported\n\nxFlasher requires Microsoft Windows Vista Service Pack 2 or later", "Can't", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
        }
        ...
        private void initTimerUpd(object source, EventArgs e)
        {
            ...
            else
            {
                ready = true;
                initTimer.Stop();
                if (!inUse) MainWindow.mainWindow.xFlasherBusy(-1);
                waiting = false;
            }
        }
        ...
        public void getFlashConfig()
        {
            ...
            Thread ftdiThread = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        waiting = true;
                        MainWindow.mainWindow.xFlasherBusy(-2);
        ...
        public int getFlashConfigActual(bool auto = false)
        {
            ...
            if (result == 0 || result == -3 || result == -4)
            {
                ...
                if (flashconf == "C0462002")
                {
                    ...
                    {
                        Console.WriteLine("");
                        MessageBox.Show("Unable to read/write eMMC type console in SPI mode\n\nPlease switch to eMMC mode", "Can't", MessageBoxButton.OK, MessageBoxImage.Error);
                        return 1;
                    }
                }
                else if (result == -4)
                ...
                try
                {
                    ...
                    if (!skipboardcheck)
                    {
                        ...
                        if (size == 0) // Auto Detect Dump Size From Board
                        {
                            ...
                            {
                                MainWindow.mainWindow.Dispatcher.BeginInvoke((Action)(() => MainWindow.mainWindow.xFlasherNandSelShow(1, true))); // Ask BB
                                return;
                            }
                            else
                            {
                                MainWindow.mainWindow.Dispatcher.BeginInvoke((Action)(() => MainWindow.mainWindow.xFlasherNandSelShow(1))); // Ask
                                return;
                            }
                        }
                    }
                    ...
                    for (i = 1; i <= iterations;)
                    {
                        ...
                        {
                            Thread.Sleep(1000);
                            MainWindow.mainWindow.xFlasherInitNand(i);
                            Thread.Sleep(1000);
                        }
                        ...
                        {
                            if (MessageBoxResult.Cancel == MessageBox.Show("File already exists, it will be DELETED! Press OK to continue", "About to overwrite a nanddump", MessageBoxButton.OKCancel, MessageBoxImage.Warning))
                            {
                        ...
                        MainWindow.mainWindow.xFlasherBusy(1);
                        ...
                        MainWindow.mainWindow.xFlasherBusy(0);
        ...
        public void writeNandAuto()
        {
            ...
            {
                MessageBox.Show("Nand is not a valid size", "Can't", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
                ...
                try
                {
                    ...
                    if (mode == 0 && filename != "erase" && !skipboardcheck)
                    {
                        if (flashconf == "00023010" || flashconf == "00043000" || flashconf == "01198010")
                        {
                            if (size != 16)
                            {
                                if (MessageBoxResult.No == MessageBox.Show("You are attempting to write a " + size + "MB Nand to a board with a 16MB Flash Config.\n\nAre you sure that you want to do that?", "Warning", MessageBoxButton.YesNo, MessageBoxImage.Warning))
                                {
                                    Console.WriteLine("xFlasher: Cancelled");
                                    Console.WriteLine("");
                                    return;
                        ...
                        else if (flashconf == "01198030")
                        {
                            if (size != 64)
                            {
                                if (MessageBoxResult.No == MessageBox.Show("You are attempting to write a " + size + "MB Nand to a board with a 64MB Flash Config.\n\nAre you sure that you want to do that?", "Warning", MessageBoxButton.YesNo, MessageBoxImage.Warning))
                                {
                                    Console.WriteLine("xFlasher: Cancelled");
                                    Console.WriteLine("");
                                    return;
                        ...
                        else if (flashconf == "008A3020" || flashconf == "00AA3020")
                        {
                            if (size == 16)
                            {
                                if (MessageBoxResult.No == MessageBox.Show("You are attempting to write a " + size + "MB Nand to a board with a 64/256/512MB Flash Config.\n\nAre you sure that you want to do the things?", "Warning", MessageBoxButton.YesNo, MessageBoxImage.Warning))
                                {
                                    Console.WriteLine("xFlasher: Cancelled");
                                    Console.WriteLine("");
                                    return;
                        ...
                        {
                            if (MessageBoxResult.No == MessageBox.Show("I think the correct size is " + size + "MB.\n\nDo you want to continue as " + size + "MB?", "Unrecognized Flash Config", MessageBoxButton.YesNo, MessageBoxImage.Warning))
                            {
                                MainWindow.mainWindow.Dispatcher.BeginInvoke((Action)(() => MainWindow.mainWindow.xFlasherNandSelShow(2))); // Ask
                                return;
                            }
                        }
                    }
                    ...
                    if (filename == "erase")
                    {
                        MainWindow.mainWindow.xFlasherBusy(3);
                    ...
                    else if (result == 0)
                    {
                        ...
                        {
                            Thread.Sleep(500);
                            MainWindow.mainWindow.afterWriteEccCleanup();
                        }
                    }
                    else if (result == -2)
                    {
        ...
        private void getBlocks(int start, int length)
        {
            ...
            while (inUse)
            {
                ...
                if (blocks >= 0)
                {
                    ...
                    MainWindow.mainWindow.xFlasherBlocksUpdate(blocks.ToString("X"), ((blocks - start) * 100) / length);
                }
                else MainWindow.mainWindow.xFlasherBlocksUpdate("Initializing", 0);
                Thread.Sleep(40);
            }
        }
